...
import { format, startOfMonth, endOfMonth, eachDayOfInterval, getDay, isSameDay, parseISO, isSameMonth } from "date-fns";
...
const Index = () => {
  ...
  const formatDateRangeForCard = (startDateString: string, endDateString: string) => {
    const startDate = parseISO(startDateString);
    const endDate = parseISO(endDateString);

    if (isSameDay(startDate, endDate)) {
      return `${format(startDate, 'MMM dd, yyyy')} · ${format(startDate, 'p')} - ${format(endDate, 'p')}`;
    }
                        ...
                        <Card key={event.id} className="border-campus-border hover:shadow-lg transition-shadow">
                          ...
                          <CardContent className="space-y-3">
                            <div className="space-y-2 text-sm text-muted-foreground">
                              <div className="flex items-center gap-2">
                                <Calendar className="h-4 w-4" />
                                <span>{formatDateRangeForCard(event.start_date, event.end_date)}</span>
                              </div>
                              <div className="flex items-center gap-2">
